...
import facData from '../hod/staticdata';
...
export default function StaticFacultyLog() {
  ...
  // Process the facData for the logged-in faculty when user context is available
  useEffect(() => {
    ...
    // Determine ERP ID from user object, falling back to username if needed
    const userErpId = user.ErpStaffID ?? user.erpStaffID ?? user.staffId ?? user.username;
    // Guard against missing ERP ID
    if (userErpId == null) {
      setError('User ERP ID not found');
      setLoading(false);
      return;
    }
    // Filter static data by ERP staff ID from the logged-in user
    const filteredDataArray = facData.filter(item => item.ErpStaffID.toString() === userErpId.toString());
    if (filteredDataArray.length === 0) {
      setError('No attendance data found for current user');
      setLoading(false);
      return;
    }
    const staffName = filteredDataArray[0].StaffName; // derive exact name from staticdata
    try {
      const processedData = processRawData(filteredDataArray);
      setData(processedData);
      generateSummaryData(processedData);
      // Default to the current user for selection
    ...
    <div className={`p-6 ${expandedView ? 'max-w-full' : 'max-w-6xl'} mx-auto bg-white rounded-lg shadow-lg transition-all duration-300`}>
      ...
      {data && !loading && (
        <div className="space-y-8">
          ...
          {/* Date Selection */}
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-5 rounded-lg border border-indigo-100 shadow-sm w-full max-w-xs">
